...
MAQUINAS_MAP = {
    # === ESTAMPADO ===
    "P23": "PRENSAS PROGRESIVAS", "P24": "PRENSAS PROGRESIVAS", "P25": "PRENSAS PROGRESIVAS",
    "P26": "PRENSAS PROGRESIVAS GRANDES", "P27": "PRENSAS PROGRESIVAS GRANDES",
    "BAL02": "BALANCIN", "BAL03": "BALANCIN", "BAL05": "BALANCIN", "BAL06": "BALANCIN",
    "BAL07": "BALANCIN", "BAL08": "BALANCIN", "BAL09": "BALANCIN", "BAL10": "BALANCIN",
    "P11": "HIDRAULICAS", "P16": "HIDRAULICAS", "P17": "HIDRAULICAS", "P18": "HIDRAULICAS",
    "P15": "MECANICAS", "P19": "MECANICAS", "P20": "MECANICAS", "P21": "MECANICAS", "P22": "MECANICAS",
    "GOF01": "Gofradora",
    # === SOLDADURA ===
    "SOP03": "PRP", "SOP05": "PRP", "SOP08": "PRP", "SOP09": "PRP", "SOP10": "PRP",
    "SOP17": "PRP", "SOP18": "PRP", "SOP19": "PRP", "SOP20": "PRP", "SOP22": "PRP",
    "SOP23": "PRP", "SOP24": "PRP", "SOP25": "PRP",
    "REM20": "Remachadora",
    "DOB01": "DOBLADORA", "DOB02": "DOBLADORA", "DOB03": "DOBLADORA", "DOB04": "DOBLADORA",
    "DOB05": "DOBLADORA", "DOB06": "DOBLADORA",
...
GRUPOS_SOLDADURA = ['PRP', 'Remachadora', 'DOBLADORA', 'CELDA SOLDADURA', 'CELDA SOLDADURA RENAULT']

# ==========================================
# 1. CONFIGURACIÓN Y ESTILOS
# ==========================================
st.set_page_config(
    page_title="Generador de Reportes PDF - Fumiscor", 
    layout="wide", 
    page_icon="📄"
)
...
def redactar_resumen_ejecutivo(pdf, area, df_pdf, df_oee_target):
    ...
    # Usar los grupos correctos de Fumiscor
    grupos_area = GRUPOS_ESTAMPADO if area.upper() == 'ESTAMPADO' else GRUPOS_SOLDADURA
    mejores_oee = {}
    
    for g in grupos_area:
        m = get_metrics_direct(g, df_oee_target)
        if m['OEE'] > 0: mejores_oee[g] = m['OEE']
    
    texto_oee = ""
    if mejores_oee:
        mejor_maq = max(mejores_oee, key=mejores_oee.get)
        texto_oee = f"Durante este periodo, el grupo con mejor rendimiento general fue '{mejor_maq}' con un OEE de {mejores_oee[mejor_maq]:.1%}. "
...
def crear_pdf(area, label_reporte, oee_target_df, op_target_df, ini_date, fin_date, p_tipo):
    # COLORES PARA FUMISCOR
    if area.upper() == "ESTAMPADO":
        theme_color = (0, 128, 128) # Teal / Cerceta
        chart_bars = ['#008080', '#66B2B2', '#B2D8D8']
    else:
        theme_color = (178, 34, 34) # Crimson / Rojo Oscuro
    ...
    # Filtrar para la planta actual
    df_pdf = df_pdf_raw[df_pdf_raw['Fábrica'].str.contains(area, case=False, na=False)].copy()
    
    # Asignar grupo a cada fila (por si en un futuro necesitas graficar por grupo)
    ...
    # Detalle por GRUPO Fumiscor
    pdf.ln(3)
    pdf.set_font("Arial", 'B', 10)
    pdf.cell(0, 6, clean_text("Detalle OEE por Grupo de Maquinas:"), ln=True)
    
    grupos_area = GRUPOS_ESTAMPADO if area.upper() == 'ESTAMPADO' else GRUPOS_SOLDADURA
    for g in grupos_area:
        m_g = get_metrics_direct(g, oee_target_df)
        print_pdf_metric_row(pdf, f"   > {g} ", m_g)
    pdf.ln(3) 
    ...
    if col_inicio and col_fin and not df_pdf.empty:
        ...
        for (maq, fecha), g in df_pdf.groupby(['Máquina', 'Fecha_Filtro']):
            g_ini = g[col_inicio].apply(parse_time_to_mins).dropna()
            g_fin = g[col_fin].apply(parse_time_to_mins).dropna()
            ...
            # Ajuste en caso de que el turno cruce la medianoche
            if max_f < min_i and (min_i - max_f) > 720:
                max_f += 1440
                
            total_span = max_f - min_i
            tiempos_list.append({'Máquina': maq, 'Inicio': min_i, 'Fin': max_f, 'Total': total_span})
        ...
        if not df_horarios.empty:
            if p_tipo == "Diario":
                ...
                col_h1, col_h2, col_h3 = "Hora Inicio", "Hora Cierre", "Tiempo Operativo"
            else:
                ...
                pdf.ln(2)
                col_h1, col_h2, col_h3 = "Hora Inicio Promedio", "Hora Cierre Prom.", "Apertura Promedio"
    ...
    if not df_fallas_area.empty:
        check_space(pdf, 110)
        print_section_title(pdf, "3. Analisis de Fallas (Diagrama Pareto)", theme_color)
        ...
        pdf.set_font("Arial", 'B', 10)
        pdf.set_text_color(*theme_color)
        pdf.cell(0, 7, clean_text("Registro Detallado de Fallas:"), ln=True)
        ...
        for maq in maquinas_con_fallas:
            ...
            for _, row in df_maq.iterrows():
                ...
                pdf.cell(80, 6, clean_text(str(row['Nivel Evento 6'])[:55]), border='B')
    ...
    if not op_target_df.empty:
        ...
        if col_perf and col_area:
            ...
            if area.upper() == "ESTAMPADO":
                imprimir_cuadro_perfo("Operarios ESTAMPADO", df_est, (0, 128, 128))
            elif area.upper() == "SOLDADURA":
                imprimir_cuadro_perfo("Operarios SOLDADURA", df_sol, (178, 34, 34))
